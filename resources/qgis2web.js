...
map.getView().fit([15693847.610245, 4620515.516651, 15715679.831607, 4631559.086332], map.getSize());
    ...
    map.forEachFeatureAtPixel(pixel, function (feature, layer) {
        if (layer && feature instanceof ol.Feature && (layer.get("interactive") || layer.get("interactive") === undefined)) {
            ...
            if (typeof clusteredFeatures !== "undefined") {
                ...
                if (doPopup) {
                    popupText += '<li><table>';
                    popupText += '<a>' + '<b>' + currentFeature.get(currentFeatureKeys[6]) + '</b>' + '</a>';
                    popupText += createPopupField(currentFeature, currentFeatureKeys, layer);
                    popupText += '</table>';
                }
            }
...
function createPopupField(currentFeature, currentFeatureKeys, layer) {
    ...
    for (var i = 0; i < currentFeatureKeys.length; i++) {
        if (currentFeatureKeys[i] != 'geometry') {
            ...
            //フィールドエイリアスの取得
            var fieldAlias = layer.get('fieldAliases')[currentFeatureKeys[i]] || currentFeatureKeys[i];
            popupField += fieldAlias + '：';

            //属性値の処理（テキスト・リンク・画像・動画等）
...
attributionList.innerHTML = `
    <a href="https://front.geospatial.jp/">登記所備付地図データ</a> &middot;
    ...
    ©2024 <a href="https://x.com/8712_mitakedou">みたけどう</a>
`;
